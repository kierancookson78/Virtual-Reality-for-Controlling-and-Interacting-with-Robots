...
public class ArmTrackingLeftHand : MonoBehaviour
{
    ...
    void Update()
    {
        if (Time.time - lastRotationTime >= rotationCooldown)
        {
            ...
            generalSettings.AdjustShoulderPitchLeftInit(devicePositionValue.y * Mathf.Rad2Deg);
            generalSettings.AdjustShoulderRollLeftInit(devicePositionValue.x * Mathf.Rad2Deg);
